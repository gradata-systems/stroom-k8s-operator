...
func (r *DatabaseServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err = r.Get(ctx, types.NamespacedName{Name: dbServer.Name, Namespace: dbServer.Namespace}, &foundSecret)
	if err != nil && errors.IsNotFound(err) {
		...
		if err != nil {
			logger.Error(err, "Failed to create new StatefulSet", "Namespace", resource.Namespace, "Name", resource.Name)
	...
	err = r.Get(ctx, types.NamespacedName{Name: dbServer.Name, Namespace: dbServer.Namespace}, &foundStatefulSet)
	if err != nil && errors.IsNotFound(err) {
